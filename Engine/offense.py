...
def calculate_offensive_ratings(team_name: str, year: int) -> dict:
    ratings = get_team_positional_ratings(team_name, year)

    rushing_offense = ratings.get("RB", 0)
    passing_offense = ratings.get("QB", 0)
    total_offense = rushing_offense + passing_offense  # Can weight this if needed

    return {
        "Rushing Offense": rushing_offense,
        "Passing Offense": passing_offense,
